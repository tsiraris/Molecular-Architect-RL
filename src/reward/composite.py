"""
===========================================================================================
Stage-2/3 Terminal-Reward Combiner & Diversity Archive
===========================================================================================

This module defines the advanced reward system, which upgrades the Phase-1 generative baseline 
...
this script wraps that property term (P) with new signals: Affinity (A), Diversity (D), and 
a Warhead bonus (for covalent targeting).
...
The overall reward calculation follows this mathematical formulation:
    total_unit = w_property * P 
               ...
               - w_diversity * D                          # Penalizes similarity (anti mode-collapse)
               + w_warhead * warhead_pen                  # Bonus for containing a warhead functional group
...
Anti-Hacking Guards (By Design):
    ...
    3. The Diversity Penalty (`D`): Directly attacks the mode collapse measured in the Stage-1 run 
       (which suffered from 0.02 uniqueness). A molecule identical to what the policy keeps 
       emitting is now strictly worth less.
"""
...
def default_reward_cfg() -> Dict:
    """
    Provides the default hyperparameter configuration for the Stage-2/3 reward system.
    
    Returns a dictionary of weights and toggles. It manages backward compatibility with 
    Stage-1 behaviors while exposing new tuning parameters for Affinity, Diversity, and Warheads.
    ...
    Example:
        ...
        >>> print(cfg["use_warhead"])
        False
    """
    # -----------------------------------------------------------------------------------------
    # Default Configuration Factory
    # Establish baseline tuning parameters, enabling backward compatibility.
    # -----------------------------------------------------------------------------------------
    return {                                                                                        # Begin returning the statically defined default configuration dictionary
        ...
        "use_warhead": False,                                                                       # Toggle for optionally adding a warhead bonus to the reward if the generated molecule contains a warhead functional group
        "w_warhead": 0.15,                                                                          # Warhead bonus weight defining the magnitude of the covalent incentive
    }                                                                                               
...
def combine(base: float, soft: float, curriculum_ratio: float, cfg: Dict,
            ...
            diversity_pen: float = 0.0, warhead_pen: float = 0.0) -> Tuple[float, Dict]:
    """
    Aggregates the gated property base with affinity, diversity, and warhead signals into a single reward.
    ...
    4. Computes the `total_unit` as a weighted linear combination of P, A, D, and Warhead signals.
    ...
    Args:
        ...
        warhead_pen (float, optional): Pre-computed warhead bonus penalty. Defaults to 0.0.
        
    Returns:
        Tuple[float, Dict]: The final aggregated scalar reward, alongside a tracking dict of components.
    ...
    """
    # -----------------------------------------------------------------------------------------
    # Base Normalization & Term Preparation
    ...
    # -----------------------------------------------------------------------------------------
    # Aggregation & Final Scaling
    # Linearly combine the components and apply the rigid synthesizability gate.
    # -----------------------------------------------------------------------------------------
    total_unit = (cfg["w_property"] * P                                                             # Multiply the normalized property term by its defined configuration weight
                  ...
                  + cfg.get("w_warhead", 0.0) * warhead_pen)                                        # Apply the pre-computed warhead_pen (=1.0 when use_warhead=True AND has_warhead(mol), else 0.0). Uses .get() for backward-compat with Stage-1/2 cfgs
    ...
    info = {"P": P, "A": A, "D": D, "aff_hat_z": aff_hat_z, "aff_unc_z": aff_unc_z,                 # Package the normalized component scalars, raw z-scores, and uncertainties into an analytical tracking dictionary
            "diversity_pen": diversity_pen, "warhead_pen": warhead_pen}                             # Append the specific diversity and warhead signals to the tracking dictionary
